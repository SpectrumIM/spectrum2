...
void ServerFromClientSession::handlePasswordValid() {
	if (!isInitialized()) {
		getXMPPLayer()->writeElement(std::shared_ptr<AuthSuccess>(new AuthSuccess()));
		authenticated_ = true;
...
void ServerFromClientSession::handlePasswordInvalid(const std::string &error) {
	if (!isInitialized()) {
		...
		if (!error.empty()) {
			std::shared_ptr<StreamError> msg(new StreamError(StreamError::UndefinedCondition, error));
...
#else
void ServerFromClientSession::handleElement(std::shared_ptr<Element> element) {	
#endif
	if (isInitialized()) {
