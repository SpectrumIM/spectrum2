...
static const char *howtoplay =
    ...
    "though you had typed them individually at seperate prompts.  For "
    "example,\n"
    "you could type all of the following at once, before pressing the ENTER "
    "(or\n"
    "RETURN) key:\n"
    "\n"
    "> TURN ON THE LIGHT. TAKE THE BOOK THEN READ ABOUT THE JESTER IN THE "
    "BOOK\n"
    "\n"
    "If the story doesn't understand one of the sentences on your input line,\n"
    "or if an unusual event occurs, it will ignore the rest of your input "
    "line.\n"
    "\n"
    ...
    "\n"
    "There are three kinds of questions you can ask:  \"WHERE IS "
    "(something)\",\n"
    ...
    "> WHERE IS EVERYBODY?\n"
    "\n"
    "When you meet intelligent creatures, you can talk to them by typing "
    "their\n"
    "name, then a comma, then whatever you want to say to them.  Here are "
    "some\n"
    "examples:\n"
    "\n"
    "> JESTER, HELLO\n"
    ...
    "than one command on the same input line.  Keep in mind, however, that "
    "many\n"
    "creatures don't care for idle chatter; your actions will speak louder "
    "than\n"
    "your words.  \n";
...
public:
  FrotzNetworkPlugin(Config *config, const std::string &host, int port)
      : BoostNetworkPlugin(config, host, port) {
  }

  void handleLoginRequest(const std::string &user,
    ...
    np->handleBuddyChanged(user, "zcode", "ZCode", groups,
                           pbnetwork::STATUS_ONLINE);
    // 			sleep(1);
    // 			np->handleMessage(np->m_user, "zork", first_msg);
  }

  void handleLogoutRequest(const std::string &user,
                           const std::string &legacyName) {
  ...
  void handleMessageSendRequest(const std::string &user,
                                const std::string &legacyName,
    ...
    if (message.find("start") == 0) {
      ...
      np->handleMessage(
          user, "zcode",
          "Games are saved/loaded automatically. Use \"restart\" to restart "
          "existing game. Emulator commands are:\nstart "
          "<game>\nstop\nhowtoplay\n\nList of games:\n" +
              games);
    }
  }

  void handleJoinRoomRequest(const std::string &user, const std::string &room,
                             const std::string &nickname,
                             const std::string &password) {}

  void handleLeaveRoomRequest(const std::string &user,
