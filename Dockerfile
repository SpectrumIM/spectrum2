...
RUN curl https://packages.spectrum.im/packages.key | sudo gpg --dearmor -o /etc/apt/trusted.gpg.d/spectrumim.gpg
...
RUN echo "deb [signed-by=/etc/apt/trusted.gpg.d/spectrumim.gpg] https://packages.spectrum.im/spectrum2/ bullseye main" | tee -a /etc/apt/sources.list
RUN curl -fsSL https://packages.spectrum.im/packages.key | sudo gpg --dearmor -o /etc/apt/trusted.gpg.d/spectrumim.gpg
RUN apt-get update -qq
